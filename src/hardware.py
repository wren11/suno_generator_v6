...
def print_hardware_report() -> None:
    """Print formatted hardware and multi-GPU acceleration report."""
    hw = get_hardware_info(refresh=True)
    print("=" * 65)
    print("  HARDWARE ACCELERATION & MULTI-GPU TOPOLOGY REPORT")
    print("=" * 65)
    print(f"Status:             {hw.get('summary', 'Unknown')}")
    print(f"CUDA Available:     {hw.get('cuda_available')}")
    print(f"Device Count:       {hw.get('device_count')}")
    print(f"Multi-GPU Active:   {hw.get('multi_gpu')}")
    print(f"Dual / SLI Setup:   {hw.get('is_sli_or_dual')}")
    print(f"Total VRAM:         {hw.get('total_vram_gb', 0.0)} GB")
    print(f"Recommended Dtype:  {hw.get('recommended_dtype', 'fp32')}")
    print(f"Primary Device:     {hw.get('primary_device', 'cpu')}")
    if hw.get("devices"):
        print("\nPhysical Devices:")
        for idx, d in enumerate(hw["devices"]):
            vram = f"({d.get('vram_gb')} GB)" if "vram_gb" in d else ""
            print(f"  [{idx}] {d.get('name', 'NVIDIA GPU')} {vram}")
    print("=" * 65)
